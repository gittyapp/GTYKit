...
@implementation GTYReleaseEvent

- (id)initWithDictionary:(NSDictionary *)dictionary {
	if (self = [super initWithDictionary:dictionary]) {
		NSDictionary *payload = [dictionary mps_dictionaryForKey:@"payload"];
		self.action = [payload mps_stringForKey:@"action"];
		self.theRelease = [[GTYRelease alloc] initWithDictionary:[dictionary mps_dictionaryForKey:@"release"]];
	}
	return self;
}
...
- (NSString *)title {
	return [NSString stringWithFormat:@"%@ %@ released %@ at %@", self.actor.login, self.action, self.theRelease.name, self.repository.name];
}
